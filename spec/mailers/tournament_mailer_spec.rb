...
describe TournamentMailer do
  ...
  describe '#administration' do
    let(:tournament) { create :tournament }
    subject(:mail) { TournamentMailer.administration tournament }
    ...
    describe 'in plain' do
      ...
      it { should match %r(#{Regexp.escape tournament_url(tournament, a: tournament.admin)}) }
    end

    describe 'in html' do
      subject { message_part mail, 'html' }

      it { should have_selector %Q(a[href="#{tournament_url(tournament, a: tournament.admin)}"]) }
    end
  end
end
