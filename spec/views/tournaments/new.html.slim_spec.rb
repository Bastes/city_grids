...
describe 'tournaments/new.html.slim' do
  ...
  it { should have_selector(%Q(#tournament-new h2.city), text: city.name) }

  specify 'the tournament form' do
    within %Q(#tournament-new form[action="#{city_tournaments_path(city)}"][method="post"]) do |form|
      ...
      form.should have_selector %Q(input[name="tournament[ends_at_date]"][value="#{I18n.l tournament.ends_at, format: '%Y-%m-%d'}"][type="date"])
      form.should have_selector %Q(input[name="tournament[ends_at_time]"][value="#{I18n.l tournament.ends_at, format: '%H:%M'}"][type="time"])
      form.should have_selector %Q(input[name="tournament[places]"][value="#{tournament.places}"][type="number"][min="1"])
